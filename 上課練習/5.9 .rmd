...
```{r}
example[c("name","birthday","bloodType")]
...
str_sub(c("410583021","410873002"),5,6)
```

地址幾號: “大學號151號”，只知道號碼在「號」這字前面，但它會是第幾個位元不知道，因為每個地址的長短不一。
...
* target pattern:指對取出字串的描述。
如“108”,“98”是我們想取出的字串。我們可以說目標是，
「0-9的字元2-3個」: [:digit:]{2,3}或[0-9]{2,3}

* target location description:指target位置的情境描述。
如“民國108年”,“民國98年”，我們可以說，
...
str_view_all(Ids, "[AC]")
str_detect(Ids, "[AC]") # 偵測 出現A、C，TRUE/FLASE
...
```{r}
strip1 <- strip2 <- strip3 <- "[0-9]" #3個strip都是0到9，另類打法
strip4 <- "號"
...
str_view_all(addresses,"[0-9]+") #有問題，樓號也會選到
```

4.6.3 了解如何regex形式的描述你的target後(擷取出的資料)，接下來我們進一步學習如何更準確描述target在整個字串的位置。
...
str_replace(
  ...
  as.character(westernYear) # 要替換的內容，因為str_replace只能用character，故要as.character
) -> twDate_reformat
print(twDate_reformat)


lubridate::ymd(twDate_reformat) #lubridate會把無相關的文字去除
```

#HW 康熙元年是西元1662年，請將“康熙23年5月6日”轉成date class。（假設月日已是西曆月日）
```{r}
Kangxi_burthday <- "康熙23年5月6日"
Kangxi_regex    <- "[0-9]+"
Kan_regex       <- "康熙"
gxi_regex       <- "年"
Kangxi_pattern <- glue::glue(
  "(?<={Kan_regex}){Kangxi_regex}(?={gxi_regex})"
  )
...
KangxiYear <- as.integer(Kangxi_year)+1662

str_replace(
  Kangxi_burthday,
  Kangxi_pattern,
  as.character(KangxiYear)
) ->  Kangxi_reformat
print(Kangxi_reformat)

lubridate::ymd(Kangxi_reformat)
```
...
```{r}
midtermFilename1 <- c(midtermFilename,"exam149975013.Rmd","499770121stExam.Rmd")

# 1.
pattern <- "[43][0-9]{8}"
str_extract(midtermFilename1,pattern)  
...
# 3.
midtermFilename2 <- c(
  midtermFilename1, "exam149975013.Rmd","499770121stExam.Rmd"
)
# 3.1
