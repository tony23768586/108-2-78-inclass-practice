...
```{r}
# 各學屆2年級人數
table(transcript100to103$`學屆`)  #table 可以分辨資料，然後簡單分類
# 各學屆2年級成績大於85人數
...
```{r}
example$birthday
class(example$birthday)  #date
typeof(example$birthday) #double 可以比
```

```{r}
print(example[c("name","birthday")])

pick_after98 <- example$birthday >= lubridate::ymd("1998-01-01") #不能直接打字串"1998-01-01" 錯誤的
...
###此時會發生 ‘>=’ not meaningful for factors ，表示此時的類別資料不能排序比大小
...
#ordered 前提是levels的排序是我要的，藉由ordered讓電腦知道上述排序，是可以排序的類別資料
```

如果levels的排序不是你要的，則可在設定為factor時即把levels重新排好，並ordered=T:
```{r}  
#跟前面的chunk是同樣的方法
example$allowance <-
  factor(
...
sum(list_victimAges_female$`數目`) #資料中若出現NA，則sum會呈現NA
sum(list_victimAges_female$`數目`, na.rm=T)
#na.rm=T, rm表示remove
```

### HW 請將list_victimAges_female各元素的class做合理設定。
```{r}
...
### HW 將levels順序改成: 不詳，總計，0_5歲，12_17歲，…，70歲以上。
```{r}
...
str(transcript100to103)
```
## HW 創造可選出來自法商學院的「要/不要」向量，pick_lawBusiness。

可用factor先檢查是否存在類似法學院或商學院的資料名稱被忽略，再用levels確認
```{r}
levels(factor(transcript100to103$`學院`))

#unique函數 顯示每個資料名稱(只出現一次)
...
R還有一種特別的缺失資料NaN (即not a number)，出現在沒有定義的數學運算上，即不會出現在字串上，如：
```{r}
0/0
log(-10)
```

## HW 創立 可選出缺失資料的「要/不要」向量pick_na, 並計算有多少筆缺失。
```{r}
...
#可以直接sum(pick_na)，是因為logical，NA=TRUE=1
print(total_na)
```
...
常有同學會有以下錯誤寫法:
```{r}
pick_loveFood <- example$hobby == "美食" # == 都要一樣
pick_loveFood  #會出現FALSE，所以要用字元檢測
```
...
pick_afternoonRoutes <-
  stringr::str_detect(
    ...
    "下午"  
#factor是用數字在存，但遇到str_detect則會以螢幕顯示的資料來存
  )
...
在Arguments出現 = ，表示該input 是預設值可寫可不寫
```{r}
fruit <- c("apple", "banana", "pear", "pinapple")
str_detect(fruit, "a")
str_detect(fruit, "^a") 
#Regular Expression 字元規律描述，^a  找以a字元開頭的字元
str_detect(fruit, "a$")

str_detect(fruit, "a$", negate = T) #同下
!str_detect(fruit, "a$")
